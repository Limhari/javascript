...
//3. 해당 메인메뉴와 서브메뉴의 영역을 떠나면(mouseleave == mouseout이건 잘 안씀)
...
// nav라는 객체를 만듬
    ...
    navFn:function(){
        ...
        // console.log($mainBtn)
        // console.log($sub) 몇개있나 확인

        //메인버튼 클릭 추가(add) 이벤트(event) 리스너{listener} 등록
        //각가로[0] 꼭써야함 아이디는 안써줘도됨
        
        //메인0 버튼
        //버튼 타겟 이란 자기자신 EventTarget  ==> $mainBtn[0]
        $mainBtn[0].addEventListener('mouseenter', function(event){            
            event.preventDefault();
           // this.classList.add('on'); //addClass('on') = 얘는 제이쿼리
        //    classList는 클래스 이름 /거기에 추가하고 제거해라 
            $mainBtn[0].classList.add('on'); //this 안쓰고 쓰는 방법
           ...
           //버튼 타겟 이란 자기자신 EventTarget  ==> $mainBtn[1]
        ...
        //버튼 타겟 이란 자기자신 EventTarget  ==> $mainBtn[2]
        ...
        //버튼 타겟 이란 자기자신 EventTarget  ==> $mainBtn[3]
